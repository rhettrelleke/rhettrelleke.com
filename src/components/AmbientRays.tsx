...
export default function AmbientRays() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Config for both bursts (tweak values per side)
      const bursts = [
        { x: state.w * 0.77, y: state.h * 0.062, rays: 20, baseFactor: 0.07, maxFactor: 0.22, minPulse: 0.35, lineWidth: 3, shadowBlur: 10, speed: 1.5, phase: 0.6, glowAlpha: '0e' },
        { x: state.w * 0.23, y: state.h * 1.05, rays: 20, baseFactor: 0.065, maxFactor: 0.29, minPulse: 0.35, lineWidth: 3, shadowBlur: 10, speed: 2.5, phase: 0.6, glowAlpha: '0e' }
      ] as const
      ...
      // Rays
      const drawBurst = (centerX: number, centerY: number, rays: number, baseRadius: number, maxRay: number, minPulse: number, lineWidth: number, shadowBlur: number, speed: number, phase: number) => {
        for (let i = 0; i < rays; i++) {
        const a = (i / rays) * Math.PI * 2
        // Normalized oscillation 0..1
          const n = Math.sin(state.t * speed + i * phase) * 0.5 + 0.5
      ...
      bursts.forEach((b) => {
        ...
        drawBurst(b.x, b.y, b.rays, baseR, maxR, b.minPulse, b.lineWidth, b.shadowBlur, b.speed, b.phase)
      })
    }
